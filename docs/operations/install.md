...
certificate_identity="https://github.com/east-true/docklattice/.github/workflows/release.yml@refs/tags/$release_tag"
...
Mount durable state at `/var/lib/docklattice` and place the TLS certificate and
private key at `/var/lib/docklattice/tls/server.crt` and `server.key`. The image
...
prepare `/srv/docklattice/server-state` as root, copy the certificate and key into
...
```sh
server_cert=/secure/source/docklattice-server.crt
server_key=/secure/source/docklattice-server.key
server_ca=/secure/source/docklattice-server-ca.crt
server_private_ip=10.0.0.10

sudo install -d -m 0700 -o 65532 -g 65532 \
  /srv/docklattice/server-state \
  /srv/docklattice/server-state/tls
sudo install -m 0600 -o 65532 -g 65532 \
  "$server_cert" \
  /srv/docklattice/server-state/tls/server.crt
sudo install -m 0600 -o 65532 -g 65532 \
  "$server_key" \
  /srv/docklattice/server-state/tls/server.key
```
...
```sh
docker run -d --name docklattice-server --restart unless-stopped \
  -p 127.0.0.1:8080:8080 -p "$server_private_ip:8443:8443" \
  -v /srv/docklattice/server-state:/var/lib/docklattice:rw \
  "$server_image"
```
...
transferred_join_token=/secure/received/docklattice-join-token
...
docker run -d --name docklattice-agent \
  ...
  "$agent_image" agent \
  --server docklattice.internal:8443 \
  --registration-url https://docklattice.internal \
...
sudo rm /etc/docklattice/join-token

docker run -d --name docklattice-agent --restart unless-stopped \
  --user 65532:65532 --group-add "$docker_socket_gid" \
  --label io.docklattice.role=agent \
  -v /var/run/docker.sock:/var/run/docker.sock:rw \
  -v docklattice-agent-state:/var/lib/docklattice \
...
steady-state container configuration. DockLattice never deletes the host secret
...
Self-protection prevents DockLattice from replacing its own container. Upgrade
...
```sh
previous_agent_image=$(docker inspect docklattice-agent \
  --format '{{.Config.Image}}')
...
`docklattice-agent-state`. Confirm that the existing Agent ID returns Active
...
`/srv/docklattice/server-state` before changing the Server. The backup must
include the database, Identity State, and TLS material together.

```sh
previous_server_image=$(docker inspect docklattice-server \
  --format '{{.Config.Image}}')

docker stop docklattice-server
sudo tar -C /srv/docklattice \
  -czf "/srv/docklattice/server-state-before-$release_tag.tar.gz" \
  server-state
docker rm docklattice-server
```
