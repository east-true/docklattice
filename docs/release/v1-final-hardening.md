...
| 4 | P2 | The Agent process writes no diagnostics at all. Only the Server has a diagnostics writer, and an Agent's `docker logs` is empty across kills, restarts, partitions and an archive-rollback refusal — an unrecoverable condition with no local signal. An operator debugging a stuck Agent has the dashboard and nothing else. | **Fixed in code; current-revision VM evidence pending.** Agent stderr now carries bounded, structured, rate-limited lifecycle, reconnect, Docker, shutdown, and Archive-refusal diagnostics. Unit tests pin line bounds, UTF-8 validity, newline safety, secret redaction, and rollback fields. The recovery matrix now requires the real Agent log to contain `event=audit_archive_refused` and `ARCHIVE_ROLLBACK_DETECTED`. |
