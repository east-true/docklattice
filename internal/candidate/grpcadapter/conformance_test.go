...
func testFactory(t *testing.T) conformance.Factory {
	...
	return func(ctx context.Context, h transport.Handler, limits transport.Limits) (transport.Caller, func(), error) {
		...
		go func() {
			...
			accepted <- acceptResult{caller, err}
		}()
		connector := NewConnector(ConnectorConfig{Address: listener.Addr().String(), TLSConfig: clientTLS, AgentID: "agent-test", Limits: limits})
		...
		result := <-accepted
		if result.err != nil {
			_ = agent.Close(result.err)
			_ = acceptor.Close()
			return nil, nil, result.err
		}
		cleanup := func() {
			_ = result.caller.Close(nil)
			_ = agent.Close(nil)
			_ = acceptor.Close()
		}
		return result.caller, cleanup, nil
	}
}
